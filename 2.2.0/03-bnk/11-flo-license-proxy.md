# F5 Lifecycle Operator
F5 Lifecycle Operator (FLO) is seamlessly installed by using Helm with the values override file flo-value.yaml file, which includes all essential configurations for the successful installation of FLO and the necessary resources to configure BIG-IP Next for Kubernetes. During the FLO Helm installation, the IPAM operator, BnkGatewayClass custom resource definition (CRD), and BIG-IP Next for Kubernetes component CRDs are installed along with FLO. The IPAM Operator, in collaboration with the F5 IPAM Controller (FIC), manages the dual-stack IP addresses (IPv4 and IPv6) for Gateway resources.

Note: 
> **Update the operationMode to "disconnected" if direct Internet access is not available**  
> **Then execute commands described [here](../bnk-license.md#disconnected-mode) after step 10-bnk**

## F5 Lifecycle Operator values

```bash
cat <<EOF > flo-values.yaml
global:
  imagePullSecrets:
    - name: far-secret
  certmgr:
    clusterIssuer: bnk-intermediate-ca

namespace: "${f5_bnk_namespace}"
 
image:
  repository: repo.f5.com/images
  name: f5-lifecycle-operator
  pullPolicy: Always

fluentbit_sidecar:
  enabled: true
  image:
    name: f5-fluentbit
 
license:
  operationMode: f5licenseproxy
  logLevel: "info"
  jwt: ${f5_licenseJWT}
  friendlyName: BNK Cluster 1
  teemCertUrl: "https://${license_proxy_name}:30001/license-proxy/v1"
  teemEntitlementUrl: "https://${license_proxy_name}:30001/license-proxy/v1"
  teemInitialConfigUrl: "https://${license_proxy_name}:30001/license-proxy/v1"
  kid: "v1"
  alg: "RS512"
  kty: "RSA"
  modulus: "wgqDv-fuebdh_gV3wN8voRGcHGDo4YekYT78U2x-gAgxWDFFP4uIpQk9d_Hszevyr78xgFBD7RnR4FeWu7R62L1DnEEbrQYEjNI1quLtcfI5wvfdMLeyv5ZXe4-Uu21lwhvRtJCyzNRW797NPaVsMAzrcWwslHZdrljgbLaf3UX90ITyqd9MFgb7jNmRLjdzZLDqQjyYJTq2AOaAoagX0pqKkBWGlgNbiFAuV2RMUhis9Ei5NSfiD5I5ntsvp_Q_XxcJrYrJOs9uaXkeTMXa58JUeX7Tt6zLA6Ju1wxClYMyNWj4u5dG8u2yhCrucJ2_IvTs4A1BSOfAWW5TwS8BBYuY9H_OXgoqyYybeOU5YOHmszaDXfMs7FFZkjAFIfr95bprH-HW1cwREQ_eqkYxi9XmA3KCNQGLhZ_i9gRTu8MlGwYzckeJG_NOyPIa7WRw7zFe7RHcchqaCSjpWC4u-GSZ92uf-EUw2Db7OX4J8VcD_3cvCfCEzfsrSaTkLZvlkYLC-3x4eM7B2GiM7SA8l4u0DK0nrScLsOR5njd2IUUB38K39Jq-tlD5PoqLT6u0AE4IJtWN-S6uFhqzdUvExBYK19ZhTbeMmOEkMALlEPhsNUnOSXGnocxlsRYldwfCoNtpVYxlXrSnbHkuPlK8Q27yi1wO9r3RwO_OxQnUvXk"
  exponnt: "AQAB"
  x5c:
  - ""
  - ""
  - ""
  - ""
  use: "sig"
  licenseserverrootca: |
    -----BEGIN CERTIFICATE-----
    
      ... FROM FLP root CA
    rAhFEM1iQA9TG+JzeXfJ3qiGPbYTh3L05JwkEOGVhqakmisX2PNQxtDllQC7mdkX
    l19VW9/Rgz/sy4izZiTBNYbyr1P72XlW9jEFQLkYIg3sHcMVhLPQ1U9mWjoUynjD
    M+1Fvlx2e/xBk+WSyeIdTeiuqaCUXdrBAG+2qk/Y0v1VUX9B8mo6+/jlEUzdn80A
    Ov67qGiE1Onu4qEiKPr+zo+hC1ftxWjX/iyV2e+f2dmj6rW3cq0=
    -----END CERTIFICATE-----
EOF
```

## F5 Lifecycle Operator installation

Login to F5 Helm repo

```bash
cat cne_pull_64.json | helm registry login -u _json_key_base64 --password-stdin repo.f5.com
```


```bash
helm install flo oci://repo.f5.com/charts/f5-lifecycle-operator --version ${f5_flo_version} -f flo-values.yaml --namespace ${f5_bnk_namespace}
```

Check pods status

```bash
kubectl get pod -n ${f5_bnk_namespace} --watch
```

# FLO additional configuration to support Proxy

```bash
kubectl set env deployment/flo-f5-lifecycle-operator -n ${f5_bnk_namespace} HTTPS_PROXY=http://1.2.3.4:8080 NO_PROXY=10.0.0.0/8,svc.cluster.local
```


[Back](10-cwc-certificates.md)  
[Next](12-cne-instance.md)