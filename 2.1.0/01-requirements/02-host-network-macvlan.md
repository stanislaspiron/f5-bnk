...
```bash
for (( i=1; i<=${nodes[length]}; i++ )); do
echo "Configuring network on ${nodes[${i}:name]}"
...
[Service]
ExecStart=-/sbin/ip link add br-Internal address 02:f5:f5:f5:f5:f${i} link ${internal_interface} type macvlan mode bridge
